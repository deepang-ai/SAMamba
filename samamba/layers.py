...
class Selection(nn.Module):
    ...
    def __init__(self, embed_dim, gnn_type="gcn", se="gnn", k_hop=2, order="sd", **kwargs):
        ...
        if self.se == "khopgnn":
            ...
            self.structure_extractor = StructureExtractor(embed_dim, gnn_type=gnn_type,
                                                          num_layers=k_hop, **kwargs)
        self.out_proj = Mamba(d_model=self.embed_dim, d_state=16, d_conv=4, expand=2)
    ...
    def forward(self,
        ...
        if self.order == "sd":
            sub_deg = torch.ones(len(x_struct)).to(torch.long).to("cuda")
            all_deg = utils.degree(subgraph_edge_index[0], subgraph_indicator_index.shape[0]).to(torch.long)
            deg = sub_deg.index_add_(0, subgraph_indicator_index, all_deg)
        else:
            deg = utils.degree(edge_index[0], x_struct.shape[0]).to(torch.long)

        order_tensor = torch.stack([batch, deg], 1).T
        _, x_struct = utils.sort_edge_index(order_tensor, edge_attr=x_struct)

        h, mask = utils.to_dense_batch(x_struct, batch)
        out_proj = self.out_proj(h)[mask]
        return out_proj
...
class MambaEncoderLayer(nn.Module):
    def __init__(self, d_model, dim_feedforward=512, dropout=0.1
                 , batch_norm=True, pre_norm=False,
                 gnn_type="gcn", se="gnn", order="sd", k_hop=2, **kwargs):
        super().__init__()

        self.d_model = d_model
        self.dim_feedforward = dim_feedforward

        self.self_selen = Selection(self.d_model, gnn_type=gnn_type, se=se, k_hop=k_hop, order=order, **kwargs)
        self.batch_norm = batch_norm
        self.pre_norm = pre_norm
        if batch_norm:
    ...
    def forward(self, x, edge_index,
                ...
                subgraph_edge_attr=None,
                subgraph_indicator_index=None,
                edge_attr=None, degree=None, ptr=None, batch=None
                ):
